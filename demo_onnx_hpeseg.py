"""
    Script for demonstration of HPE-Seg task solving on a video (via ONNX).
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="HPE-Seg on a video (via ONNX)",
...
def main():
    ...
    sess = InferenceSession(args.model)
    ...
    for i in range(max_num_frames):
        ...
        output = sess.run(["output"], {"input": image3.astype(np.float32)})[0]

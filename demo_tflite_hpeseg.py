...
def extract_keypoints(heatmap,
                      ...
                      peak_radius=24.0):
    """
    Calculate all possible points and scores from heatmaps.
...
def convert_poses_to_pts_format(pose_entries,
    ...
    for pose_entry_i in pose_entries:
        if len(pose_entry_i) == 0:
            continue
        pts_i = np.full((18, 3), np.nan, np.float32)
        for j, keypoint_id in enumerate(pose_entry_i[:-2]):
            if keypoint_id != -1:
                pts_i[j] = all_keypoints[int(keypoint_id), 0:3]
        pts.append(pts_i)
    return np.array(pts)
...
def group_keypoints(all_keypoints_by_type,
    ...
    pts = convert_poses_to_pts_format(
        pose_entries=pose_entries,
        all_keypoints=all_keypoints)
    return pts


def _calc_pts_from_paf(net_output):
    """
    Calculation of multi-person HPE joints.

    Parameters
    ----------
    net_output : numpy.ndarray
        Output of the net with shape `1, H, W, 53`.

    Returns
    -------
    numpy.ndarray
        Joints with shape `N_Persons, N_Joints, 3`.
    """
    net_outputs_np = net_output[0].transpose(2, 0, 1)
    heatmap = net_outputs_np[1:19]
    paf = net_outputs_np[19:]
    ...
    pts = group_keypoints(all_keypoints_by_type, paf)
    return pts
...
def cv_plot_keypoints(image,
    ...
    keypoint_thresh : float, optional, default 0.3
...
def main():
    ...
    for i in range(max_num_frames):
        ...
        mask0 = (net_output[0, :, :, 0] > 0.5).astype(np.uint8) * 255
        if args.use_paf:
            pts0 = _calc_pts_from_paf(net_output)
        else:
            pts0 = _calc_pts_from_heatmap(net_output[:, :, :, 1:(1 + 18)])
